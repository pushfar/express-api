...
export default class Crypto {
	...
	static md5(s: string): string {
		// configure funcs
		let L = function (k: number, d: number) { return (k << d) | (k >>> (32 - d)) }
		let K = function (G: number, k: number) { let I: number, d: number, F: number, H: number, x: number; F = (G & 2147483648); H = (k & 2147483648); I = (G & 1073741824); d = (k & 1073741824); x = (G & 1073741823) + (k & 1073741823); if (I & d) { return (x ^ 2147483648 ^ F ^ H) } if (I | d) { if (x & 1073741824) { return (x ^ 3221225472 ^ F ^ H) } else { return (x ^ 1073741824 ^ F ^ H) } } else { return (x ^ F ^ H) } }
		let r = function (d: number, F: number, k: number) { return (d & F) | ((~d) & k) }
		let q = function (d: number, F: number, k: number) { return (d & k) | (F & (~k)) }
		let p = function (d: number, F: number, k: number) { return (d ^ F ^ k) }
		let n = function (d: number, F: number, k: number) { return (F ^ (d | (~k))) }
		let u = function (G: number, F: number, aa: number, Z: number, k: number, H: number, I: number) { G = K(G, K(K(r(F, aa, Z), k), I)); return K(L(G, H), F) }
		let f = function (G: number, F: number, aa: number, Z: number, k: number, H: number, I: number) { G = K(G, K(K(q(F, aa, Z), k), I)); return K(L(G, H), F) }
		let D = function (G: number, F: number, aa: number, Z: number, k: number, H: number, I: number) { G = K(G, K(K(p(F, aa, Z), k), I)); return K(L(G, H), F) }
		let t = function (G: number, F: number, aa: number, Z: number, k: number, H: number, I: number) { G = K(G, K(K(n(F, aa, Z), k), I)); return K(L(G, H), F) }
		let e = function (G: string) { let Z: number; let F = G.length; let x = F + 8; let k = (x - (x % 64)) / 64; let I = (k + 1) * 16; let aa = Array(I - 1); let d = 0; let H = 0; while (H < F) { Z = (H - (H % 4)) / 4; d = (H % 4) * 8; aa[Z] = (aa[Z] | (G.charCodeAt(H) << d)); H++ } Z = (H - (H % 4)) / 4; d = (H % 4) * 8; aa[Z] = aa[Z] | (128 << d); aa[I - 2] = F << 3; aa[I - 1] = F >>> 29; return aa }
		let B = function (x: number) { let k = "", F = "", G: number, d: number; for (d = 0; d <= 3; d++) { G = (x >>> (d * 8)) & 255; F = "0" + G.toString(16); k = k + F.substr(F.length - 2, 2) } return k }
		let J = function (k: string) { k = k.replace(/rn/g, "n"); let d = ""; for (let F = 0; F < k.length; F++) { let x = k.charCodeAt(F); if (x < 128) { d += String.fromCharCode(x) } else { if ((x > 127) && (x < 2048)) { d += String.fromCharCode((x >> 6) | 192); d += String.fromCharCode((x & 63) | 128) } else { d += String.fromCharCode((x >> 12) | 224); d += String.fromCharCode(((x >> 6) & 63) | 128); d += String.fromCharCode((x & 63) | 128) } } } return d }

		let C = Array();
		let P: number, h: number, E: number, v: number, g: number, Y: number, X: number, W: number, V: number;
	...
	static passwordHash(text: string, salt?: string): string {
		...
		let textHashStart: string, textHashEnd: string, outHash: string;
	...
	static encodeToken(scope: string, key: string, host: string, origin: string, expire: number, JWTKey: string, AESKey: string): string {
		return Crypto.encryptAES256CBC(JWT.sign({
			...
			exp: Math.floor(Date.now() / 1000) + parseInt(expire.toString()),
			key: key,
			scope: scope
	...
	static decodeToken(scope: string, token: string, JWTKey: string, AESKey: string): string {
		const decryptedToken = Crypto.decryptAES256CBC(token, AESKey);
		if (!JWT.verify(decryptedToken, JWTKey, { algorithms: ['HS256'] })) throw Error('Unable to verify token');
