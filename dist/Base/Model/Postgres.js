 ...
 * @license MIT
 */
export default class ModelPG extends Core {
    /**
    ...
    first(where) {
        ...
        if (!where || Object.keys(where).length < 1)
            return this.db.query(`SELECT * FROM ${this.inject(this.table)} ${this.notSoftDeleted('WHERE')} ORDER BY ${this.inject(this.createdCol)} ASC LIMIT 1;`).then((res) => res.rows[0] || {});
        let q = Object.keys(where).map((w, i) => ` ${this.inject(w)} = $${i + 1} `).join(' AND ');
        let v = Object.values(where);
    ...
    last(where) {
        if (!this.createdCol)
            throw new ModelError('Must set created column in super request to use this feature.');
        if (!where || Object.keys(where).length < 1)
            return this.db.query(`SELECT * FROM ${this.inject(this.table)} ${this.notSoftDeleted('WHERE')} ORDER BY ${this.inject(this.createdCol)} DESC LIMIT 1;`).then((res) => res.rows[0] || {});
        let q = Object.keys(where).map((w, i) => ` ${this.inject(w)} = $${i + 1} `).join(' AND ');
        let v = Object.values(where);
     ...
     * @return {Promise} a resulting promise of data or error on failure
     */
    all() { return this.db.query(`SELECT * FROM ${this.inject(this.table)} ${this.notSoftDeleted('WHERE')};`).then((res) => res.rows); }
    /**
    ...
    insert(data, returning) {
        ...
        return this.db.query(`INSERT INTO ${this.inject(this.table)} (${qk}) VALUES ${qv} ${r};`, v)
            ...
            .catch((error) => {
            if (error.code == 23505)
                throw new ModelError('Cannot duplicate a unique value, value already exists', this.columns);
            throw error;
        });
    }
    /**
    ...
    delete(id, type) {
        ...
        if ((!this.softDelete && type !== 'soft') || type === 'hard')
            return this.db.query(`DELETE FROM ${this.inject(this.table)} WHERE id = $1;`, [id]).then(() => undefined);
    ...
    parseError(error) {
        ...
        if (error.code == '22P02' && error.routine == 'string_to_uuid')
            return { error: 'invalid data', detail: 'uuid' };
        if (error.code == '23505')
            return { error: 'not unique', detail: error.detail.split(')=(')[0].split('(')[1] };
