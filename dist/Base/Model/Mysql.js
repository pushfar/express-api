...
import DataTools from '../../Library/DataTools.js';
/**
 ...
 * @license MIT
 */
export default class ModelMysql extends Core {
    /**
    ...
    first(where) {
        if (!this.createdCol)
            throw new ModelError('Must set created column in super request to use this feature.');
        if (!where || Object.keys(where).length < 1)
            return this.db.query(`SELECT * FROM ${this.inject(this.table)} ${this.notSoftDeleted('WHERE')} ORDER BY ${this.inject(this.createdCol)} ASC LIMIT 1;`).then(([rows]) => rows[0] || {});
        let q = Object.keys(where).map((w, i) => ` ${this.inject(w)} = ? `).join(' AND ');
        let v = Object.values(where);
    ...
    last(where) {
        if (!this.createdCol)
            throw new ModelError('Must set created column in super request to use this feature.');
        if (!where || Object.keys(where).length < 1)
            return this.db.query(`SELECT * FROM ${this.inject(this.table)} ${this.notSoftDeleted('WHERE')} ORDER BY ${this.inject(this.createdCol)} DESC LIMIT 1;`).then(([rows]) => rows[0] || {});
        let q = Object.keys(where).map((w, i) => ` ${this.inject(w)} = ? `).join(' AND ');
        let v = Object.values(where);
    ...
    update(where, data) {
        ...
        if (!where || ['object', 'string', 'number'].indexOf(typeof where) < 0)
            throw new ModelError('Must use where criteria in update as either an ID or object containing col: value');
        if (typeof where !== 'object')
            where = { id: where };
        if (Object.keys(where).length < 1)
            throw new ModelError('Must have at least one where criteria in where object');
        let dl = 0;
        let qu = Object.keys(data).map((d, i) => {
            dl++;
            if (!this.columns || !this.columns[d] || this.columns[d].type !== 'point')
    ...
    queryWhere(args, values, chain) {
        if (Object.keys(args.where || args).length < 1)
            return '';
     ...
     */
    queryOrder(args) {
        if (!args || (!args.order && !args.orderBy))
            return '';
        ...
        for (let i = 0; i < ords.length; i++) {
            if (!ords[i].key)
                throw new ModelError(`Cannot parse order, key [${ords[i].key}] key missing`);
            if (!ords[i].direction)
                throw new ModelError(`Cannot parse order, value [${ords[i].direction}] direction missing`);
            if (!this.columns || !this.columns[DataTools.camelToSnake(ords[i].key)])
                throw new ModelError(`Cannot set order, key [${ords[i].key}] not found in data`);
    ...
    arrayOrder(args, items) {
        ...
        for (let i = 0; i < ords.length; i++) {
            items = items.sort((a, b) => {
                if (!a[ords[i].key])
                    throw new ModelError(`Cannot parse order key [${ords[i].key}], key does not exist`);
    ...
    arrayLimit(args, items) {
        if (!args || !args.limit || !Array.isArray(items) || items.length < 1)
            return items;
        let limit = Number(args.limit);
        if (!limit)
            return items;
    ...
    arrayOffset(args, items) {
        if (!args || !args.offset || !Array.isArray(items) || items.length < 1)
            return items;
        let offset = Number(args.offset);
        if (!offset)
            return items;
    ...
    mapDataToColumn(data, partial) {
        if (!this.columns)
            throw new ModelError('Cannot map data without setting columns getter in model [' + DataTools.snakeToCamel(this.table.split('.')[1]) + ']');
        // single entry
        let clean = {};
    ...
    mapDataArrayToColumn(data, partial) {
        if (partial && !data)
            return;
        if (!data || !data.length)
            throw new ModelError('Data must be an array of data objects for [' + DataTools.snakeToCamel(this.table.split('.')[1]) + ']');
        // array of data entries?
        let allClean = [];
        for (let i = 0; i < data.length; i++)
            allClean.push(this.mapDataToColumn(data[i], partial));
        return allClean;
    }
    /**
    ...
    checkColumnsStrict(data) {
        ...
        for (const key in data)
            if (this.columns[key] === undefined)
                return false;
        for (const key in this.columns)
            if (data[key] === undefined && this.columns[key].required)
                return false;
        return true;
    }
    /**
    ...
    __cleanIncommingData(data, skipId) {
        ...
        if (data.length > 0) {
            let dataArray = [];
            for (let i = 0; i < data.length; i++)
                dataArray.push(this.__cleanIncommingData(data[i]));
            return dataArray;
        }
        let cleaned = Object.assign({}, data);
        if (this.idCol && cleaned[this.idCol] && !skipId)
            delete cleaned[this.idCol];
        if (this.createdCol && cleaned[this.createdCol])
            delete cleaned[this.createdCol];
        if (this.updatedCol && cleaned[this.updatedCol])
            delete cleaned[this.updatedCol];
        if (this.deleteCol && cleaned[this.deleteCol])
            delete cleaned[this.deleteCol];
        return cleaned;
    }
    /**
    ...
    __parseQueryWhere(args, values) {
        if (typeof args !== 'object')
            throw new ModelError('Cannot set where criteria on query, structure incorrect');
        ...
        for (let k in w) {
            ...
            let key = Array.isArray(w) ? w[Number(k)].key : k;
            let val = this.__parseValue(Array.isArray(w) ? w[Number(k)] : w[k]);
            let con = this.__parseCondition(Array.isArray(w) ? (w[Number(k)].condition || '=') : '='); // default to equals
            let chn = this.__parseChain(Array.isArray(w) ? (w[Number(k)].chain || 'AND') : 'AND'); // default to AND
            ...
            if (Array.isArray(val) && val.length < 2 && con !== 'IN')
                val = val[0];
            if (Array.isArray(val) && val.length > 1 && con === '=')
                con = 'IN';
            if (con === '=' && val === null)
                con = 'IS NULL';
            if (con === '!=' && val === null)
            ...
            if (!this.columns || !this.columns[DataTools.camelToSnake(key)])
                throw new ModelError(`Cannot set where criteria query, key [${key}] not found in data`);
            ...
            if (wItem.date)
                q += ` ${c === 0 ? '' : chn} DATE(${this.inject(DataTools.camelToSnake(this.table + '.' + key))}) ${con} DATE(${bnd}) `;
            else
    ...
    __parseArrayWhere(args, items) {
        if (typeof args !== 'object')
            throw new ModelError('Cannot set where criteria on array, structure incorrect');
        if (Object.keys(args.where || args).length < 1)
            return items;
        // need to loop through the where object now and build up the query adding to string
        let w = args.where || args; // map all where
        return items.filter((item) => {
            ...
            for (let k in w) {
                // sub query? then tag on with chain type
                if (w[k] && w[k].where) {
                    f = (w[k].chain ? this.__parseChain(w[k].chain) : 'AND') === 'AND' ? f && this.__parseArrayWhere(w[k], items).length > 0 : f || this.__parseArrayWhere(w[k], items).length > 0;
                    continue;
                }
                // is this direct simple matchy or detailed chainy, map to common vars
                let key = Array.isArray(w) ? w[Number(k)].key : k;
                ...
                // check key is present in colum list
                if (!item[key])
                    throw new ModelError(`Cannot set where criteria, key [${key}] not found in data`);
                ...
                switch (chn + ' ' + con) {
                    ...
                    case 'AND !=':
                        ...
                        break;
                    case 'AND LIKE':
                        f = f && (new RegExp(`^${val.replace(/\%|\*/g, '.*')}$`)).test(item[key]);
                        break;
                    case 'AND NOT LIKE':
                        f = f && !(new RegExp(`^${val.replace(/\%|\*/g, '.*')}$`)).test(item[key]);
                        break;
                    case 'AND IN':
                        f = f && val.includes(item[key]);
                        break;
                    case 'OR =':
                        f = f || item[key] === val;
                        break;
