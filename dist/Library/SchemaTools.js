...
export default class SchemaTools {
     ...
     */
    static parseBody(request, schemaBlock, debugMessage) {
        if (!schemaBlock)
        ...
        if (!schemaBody && requestBody)
            throw new Error('Body parameters not allowed in request');
        if (schemaBody && !requestBody)
            throw new Error('Body parameters missing from request');
        if (typeof schemaBody === 'string')
            throw new Error('Schema block is detected as a string, meaning its being pointed to a swagger schema object loaded in the controller index, switch to passing in the SwaggerSchemaObjectParameter directly that is being loaded into the swagger index.');
        if (!schemaBody)
            return requestBody;
        return SchemaTools.__check(schemaBody, requestBody, false, 'body', undefined, undefined, debugMessage);
    }
    /**
     ...
     */
    static parsePathParameters(request, schemaBlock, debugMessage) {
        if (!schemaBlock)
            throw new Error('schemaBlock must be a valid SwaggerSchemaMethod or method block');
        const schemaParams = schemaBlock.parameters;
        if (!schemaParams)
            throw new Error('Path parameters not allowed in request');
        if (schemaParams && !request.parameters.path)
            throw new Error('Path parameters missing from request');
        return schemaParams
            .filter((param) => param.in === 'path')
            .reduce((pre, cur) => {
            const value = request.parameters.path[cur.name];
            return { ...pre, [cur.name]: SchemaTools.__check(cur.schema, value, false, cur.name, cur.required, undefined, debugMessage) };
        }, {});
    }
    /**
     ...
     * @returns The resulting parameter data
     */
    static parseQueryParameters(request, schemaBlock, debugMessage) {
        if (!schemaBlock)
            throw new Error('schemaBlock must be a valid SwaggerSchemaMethod or method block');
        const schemaParams = schemaBlock.parameters;
        if (!schemaParams)
            throw new Error('Path parameters not allowed in request');
        if (schemaParams && !request.parameters.query)
            throw new Error('Path parameters missing from request');
        return schemaParams
            .filter((param) => param.in === 'query')
            .reduce((pre, cur) => {
            const value = request.parameters.query[cur.name];
            return { ...pre, [cur.name]: SchemaTools.__check(cur.schema, value, false, cur.name, cur.required, undefined, debugMessage) };
        }, {});
    }
    /**
     ...
     */
    static __check(schemaParam, data, ignore, key = 'body', required, parentSchema, debugMessage) {
        // Handle oneOf - try each schema until one matches
        if ('oneOf' in schemaParam && Array.isArray(schemaParam.oneOf)) {
            return SchemaTools.__checkOneOf(schemaParam, data, ignore, key, required, parentSchema, debugMessage);
        }
        // Handle union types (type as array)
        if ('type' in schemaParam && Array.isArray(schemaParam.type)) {
            return SchemaTools.__checkUnion(schemaParam, data, ignore, key, required, parentSchema, debugMessage);
        }
        // Handle single type
        if ('type' in schemaParam && typeof schemaParam.type === 'string') {
            switch (schemaParam.type) {
                case 'string':
                    return SchemaTools.__checkString(schemaParam, data, ignore, key, required, debugMessage);
                case 'number':
                    return SchemaTools.__checkNumber(schemaParam, data, ignore, key, required, debugMessage);
                case 'boolean':
                    return SchemaTools.__checkBoolean(schemaParam, data, ignore, key, required, debugMessage);
                case 'array':
                    return SchemaTools.__checkArray(schemaParam, data, ignore, key, required, debugMessage);
                case 'object':
                    return SchemaTools.__checkObject(schemaParam, data, ignore, key, required, parentSchema, debugMessage);
            }
        }
        // no type specified
        return data;
     ...
     */
    static __checkUnion(schemaParam, data, ignore, key, required, parentSchema, debugMessage) {
        // basic check - if default and not set, return default
        if (schemaParam.default !== undefined && (data === null || data === undefined))
            return schemaParam.default;
        try {
            // basic check - if required and not set, error
            if (required && (data === null || data === undefined))
                throw new Error(`${key} union type is required.`);
                ...
                try {
                    ...
                    if (typeMatches) {
                        ...
                        if (type === 'string' || type === 'number' || type === 'boolean') {
                            ...
                            switch (type) {
                                case 'string':
                                    tempSchema = { description: schemaParam.description, type: 'string' };
                                    break;
                                case 'number':
                                    tempSchema = { description: schemaParam.description, type: 'number' };
                                    break;
                                case 'boolean':
                                    tempSchema = { description: schemaParam.description, type: 'boolean' };
                                    break;
                            }
     ...
     * @returns The validated data
     */
    static __checkOneOf(schemaParam, data, ignore, key, required, parentSchema, debugMessage) {
        try {
            // basic check - if required and not set, error
            if (required && (data === null || data === undefined))
                throw new Error(`${key} oneOf type is required.`);
            // so not required, if empty no more checks needed
            if (data === null || data === undefined)
                return data;
     ...
     */
    static __checkString(schemaParam, data, ignore, key, required, debugMessage) {
        // basic check - if default and not set, return default
        if (schemaParam.default && (data === null || data === undefined || data === ''))
            return schemaParam.default;
        try {
            // basic check - if required and not set, error
            if (required && (data === null || data === undefined || data === ''))
                throw new Error(`${key} type 'string' is required.`);
            // so not required, if empty no more checks needed
            if (data === null || data === undefined)
                return data;
            // check type
            if (typeof data !== 'string')
                throw new Error(`${key} type '${SchemaTools.__getTypeName(data)}' is not of type 'string'.`);
            // check against regex
            if (schemaParam.pattern && !(new RegExp(schemaParam.pattern, 's')).test(data))
                throw new Error(`${key} type '${schemaParam.type}' value '${data}' fails regex test '${schemaParam.pattern}'.`);
            // is it an enum?
            if (schemaParam.enum && !schemaParam.enum.includes(data))
                throw new Error(`${key} value '${data}' is not an enum string value of ${schemaParam.enum.join(', ')}`);
        }
        catch (err) {
            if (!ignore)
                throw new Error(err.message);
     ...
     */
    static __checkNumber(schemaParam, data, ignore, key, required, debugMessage) {
        // basic check - if default and not set, return default
        if (schemaParam.default !== undefined && (data === null || data === undefined))
            return schemaParam.default;
        try {
            ...
            if (typeof data !== 'number' || isNaN(data))
                throw new Error(`${key} type '${SchemaTools.__getTypeName(data)}' is not of type 'number'.`);
            // is it an enum?
            if (schemaParam.enum && !schemaParam.enum.includes(data))
                throw new Error(`${key} value '${data}' is not an enum number value of ${schemaParam.enum.join(', ')}`);
        }
        catch (err) {
            if (!ignore)
                throw new Error(err.message);
     ...
     */
    static __checkBoolean(schemaParam, data, ignore, key, required, debugMessage) {
        // basic check - if default and not set, return default
        if (schemaParam.default && (data === null || data === undefined))
            return schemaParam.default;
        try {
            // basic check - if required and not set, error
            if (required && (data === null || data === undefined))
                throw new Error(`${key} 'boolean' is required.`);
            // so not required, if empty no more checks needed
            if (data === null || data === undefined)
                return data;
            // check type
            if (typeof data !== 'boolean')
                throw new Error(`${key} type '${SchemaTools.__getTypeName(data)}' is not of type 'boolean'.`);
        }
        catch (err) {
            if (!ignore)
                throw new Error(err.message);
     ...
     */
    static __checkArray(schemaParam, data, ignore, key, required, debugMessage) {
        try {
            ...
            // basic check - if default and not set, return default
            if (schemaParam.default && (!data || !data.length))
                return schemaParam.default;
            // basic check - if required and not set, error
            if (required && (!data || !data.length))
        ...
        try {
            for (let i = 0; i < data.length; i++) {
                const validatedItem = SchemaTools.__check(schemaParam.items, data[i], ignore, `${key}[${i}]`, undefined, schemaParam, debugMessage);
     ...
     */
    static __checkObject(schemaParam, data, ignore, key, required, parentSchema, debugMessage) {
        // basic check - if default and not set, return default
        if (schemaParam.default && !data)
            return schemaParam.default;
        try {
            // basic check - if required and not set, error
            if (required && !data)
            ...
            if (typeof data !== 'object' || data === null || Array.isArray(data))
                throw new Error(`${key} type '${Array.isArray(data) ? 'array' : typeof data}' is not of type 'object'.`);
        }
        catch (err) {
            if (!ignore)
                throw new Error(err.message);
        ...
        try {
            for (const k in schemaParam.properties) {
                ...
                if (schemaParam.properties[k].items?.description === '$repeat.parent')
                    schemaParam.properties[k] = { ...parentSchema };
                // check data against schema - if ignore is false, this will throw on validation errors (wrong type, etc.)
                newData[k] = SchemaTools.__check(schemaParam.properties[k], data[k], ignore, `${key}.${k}`, schemaParam.required?.includes(k), schemaParam, debugMessage);
            }
        }
        catch (err) {
            // Re-throw validation errors when ignore is false (input validation)
