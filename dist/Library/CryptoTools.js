...
export default class Crypto {
    ...
    static md5(s) {
        ...
        let K = function (G, k) { let I, d, F, H, x; F = (G & 2147483648); H = (k & 2147483648); I = (G & 1073741824); d = (k & 1073741824); x = (G & 1073741823) + (k & 1073741823); if (I & d) {
        ...
        let e = function (G) { let Z; let F = G.length; let x = F + 8; let k = (x - (x % 64)) / 64; let I = (k + 1) * 16; let aa = Array(I - 1); let d = 0; let H = 0; while (H < F) {
            Z = (H - (H % 4)) / 4;
            d = (H % 4) * 8;
            aa[Z] = (aa[Z] | (G.charCodeAt(H) << d));
            H++;
        } Z = (H - (H % 4)) / 4; d = (H % 4) * 8; aa[Z] = aa[Z] | (128 << d); aa[I - 2] = F << 3; aa[I - 1] = F >>> 29; return aa; };
        let B = function (x) { let k = "", F = "", G, d; for (d = 0; d <= 3; d++) {
            G = (x >>> (d * 8)) & 255;
            F = "0" + G.toString(16);
            k = k + F.substr(F.length - 2, 2);
        } return k; };
        let J = function (k) { k = k.replace(/rn/g, "n"); let d = ""; for (let F = 0; F < k.length; F++) {
    ...
    static passwordHash(text, salt) {
        ...
        if (!salt)
            salt = Crypto.sha256(Math.random().toString());
        ...
        else if (saltStart > (salt.length - 1))
            outHash = Crypto.sha256(textHash + salt);
        else
            outHash = Crypto.sha256(salt + textHash);
    ...
    static decodeToken(scope, token, JWTKey, AESKey) {
        ...
        if (!JWT.verify(decryptedToken, JWTKey, { algorithms: ['HS256'] }))
            throw Error('Unable to verify token');
        let decoded = JWT.decode(decryptedToken, { complete: true });
        if (decoded.payload.scope !== scope)
            throw Error('Unable to verify token scope');
     ...
     */
    static async compareBcryptHash(stringOne, stringTwo) {
        // PHP used a slightly different format to Node, so we need to convert it to the same format
        if (stringOne.trim().startsWith('$2y$'))
            stringOne = `$2a$${stringOne.slice(4)}`;
        if (stringTwo.trim().startsWith('$2y$'))
            stringTwo = `$2a$${stringTwo.slice(4)}`;
     ...
     */
    static async makeBcryptHash(plain, saltRounds = 13) {
        if (plain == null || plain === '')
     ...
     */
    static generateApiKey(env = 'live') {
