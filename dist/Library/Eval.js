...
export default class Eval {
     ...
     */
    static execute(data, props, empty) {
        ...
        try {
            ...
            if (evalFullMatch)
                return Function(`return ${evalFullMatch[1]};`).bind(props)() || (empty !== undefined ? empty : null);
            // partial match (force to string)
            const evalPartMatch = /\$\{(.+)\}/s.test(data.toString());
            if (evalPartMatch)
                return Function(`return \`${data.toString()}\`;`).bind(props)() || (empty !== undefined ? empty : null);
        }
        catch (error) {
     ...
     * @return {Mixed} The result of the execution or the input data returned if not executable
     */
    static deepExecute(data, props, empty) {
        if (typeof data !== 'object')
            return Eval.execute(data, props, empty);
        for (const key in data)
            data[key] = Eval.deepExecute(data[key], props, empty);
        return data;
    }
}
