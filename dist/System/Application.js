...
export default class Application {
    constructor(request, type = 'express') {
        ...
        if (this._types.indexOf(type) < 0)
            throw Error('Type does not exist, please add a type of request [' + this._types.join(', ') + ']');
        this._type = type;
        // check required env vars
        if (!process.env._EAPI_CONTROLLER_PATH)
            console.warn('Controller directory not set, please set the _EAPI_CONTROLLER_PATH, defaulting to src/Controller');
        if (!process.env._EAPI_HANDLER_FILE)
            console.warn('Handler file not set, please set the _EAPI_HANDLER_FILE, defaulting to src/handler.js');
        if (!process.env._EAPI_ROUTES_FILE)
            console.warn('Routes file not set, please set the _EAPI_ROUTES_FILE, defaulting to src/routes.js');
        // get env vars
        this.globals.$environment = {};
        for (const key in process.env)
            if (key.startsWith('EAPI_'))
                this.globals.$environment[key] = process.env[key] || '';
        // get controller path
    ...
    service(s) {
        const services = !Array.isArray(s) ? [s] : s;
        for (let i = 0; i < services.length; i++)
            if (services[i].service)
                this.globals.$services[services[i].service] = services[i];
    }
    middleware(mw) {
        const middlewares = !Array.isArray(mw) ? [mw] : mw;
        for (let i = 0; i < middlewares.length; i++) {
            if (middlewares[i].start)
                this._middleware.start.push(middlewares[i]);
            if (middlewares[i].mount)
                this._middleware.mount.push(middlewares[i]);
            if (middlewares[i].in)
                this._middleware.in.push(middlewares[i]);
            if (middlewares[i].out)
                this._middleware.out.push(middlewares[i]);
            if (middlewares[i].end)
                this._middleware.end.push(middlewares[i]);
        }
    }
    middlewareInit(mw) {
        const middlewares = !Array.isArray(mw) ? [mw] : mw;
    ...
    _process(request) {
        return Promise.resolve(request)
            ...
            if (this.globals.$environment.EAPI_PATH_UNSHIFT || this.globals.$environment.PATH_UNSHIFT)
                rpath = rpath.replace(this.globals.$environment.EAPI_PATH_UNSHIFT || this.globals.$environment.PATH_UNSHIFT, '');
            if (this.globals.$environment.EAPI_PATH_SHIFT || this.globals.$environment.PATH_SHIFT)
                rpath = (this.globals.$environment.EAPI_PATH_SHIFT || this.globals.$environment.PATH_UNSHIFT) + rpath;
            // resolve name and path
            let resourcePath = rpath.split('/');
            for (let i = 0; i < resourcePath.length; i++) {
                if (!resourcePath[i])
                    continue;
                if (!resourcePath[i] || resourcePath[i].charAt(0) === '{')
                    continue;
            ...
            try {
                ...
                if (!error.exception)
                    console.warn(error.message, JSON.stringify(error.stack));
